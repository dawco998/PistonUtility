...
# JEDNOSTKI (POTWIERDZONE):
# Duration: Os Rail = bar * 10, Os IQ = mm3 * 100, Wartosci = us
# Rail Pressure: Os IQ = mm3 * 100 (saturacja na 4000=40mm3), Wartosci = bar
...
soi_balans = {1500:5.2, 1750:5.7, 2000:6.4, 2250:7.2, 2500:8.0, 2750:8.6, 3000:9.4, 3250:10.1, 3500:10.8}
...
print("="*100)
print("EOI (End of Injection) - Mazda 1.6 MZ-CD / EDC16C3 - POPRAWNE JEDNOSTKI")
print("="*100)
print()
print(f"{'RPM':<6}|{'Nm':<6}|{'IQ mm3':<8}|{'Rail':<7}|{'Dur us':<8}|{'Dur ms':<7}|{'Dur deg':<8}|{'SOI':<6}|{'EOI':<12}|{'Status'}")
print("-"*85)
...
for rpm in [1500, 1750, 2000, 2250, 2500, 2750, 3000, 3250, 3500]:
    ...
    soi = soi_balans.get(rpm, 6.0)
    eoi_val = soi - dur_deg
    
    if eoi_val > 0:
        eoi_str = f"{eoi_val:.1f} BTDC"
    else:
        eoi_str = f"{abs(eoi_val):.1f} ATDC"
    
    eoi_atdc = -eoi_val
    if 5 <= eoi_atdc <= 25: status = "OK"
    elif eoi_atdc < 5: status = "WCZESNE"
    else: status = "POZNE!"
    
    print(f"{rpm:<6}|{target_nm:<6.0f}|{iq_mm3:<8.1f}|{rail_bar:<7.0f}|{dur_us:<8.0f}|{dur_ms:<7.2f}|{dur_deg:<8.1f}|{soi:<6.1f}|{eoi_str:<12}|{status}")
